 ...
 * @return array
 */
function house_state_body_classes( $classes ) {
	...
	if ( is_singular() || is_home() ) {
		$id = ( is_home() && ! is_front_page() ) ? get_option( 'page_for_posts' ) : get_the_id();
	  	$sidebar = get_post_meta( $id, 'house-state-selected-sidebar', true );
...
add_filter( 'body_class', 'house_state_body_classes' );
